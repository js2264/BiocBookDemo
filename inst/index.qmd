...
The Docker images also include a `python` environment, named `BiocBook`, in 
which all the packages listed in `requirements.yml` are installed, and which 
`reticulate` picks up automatically in any `R` session started from the image 
(see @sec-python). 
